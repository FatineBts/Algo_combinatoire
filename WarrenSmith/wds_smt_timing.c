...
/* Global variables: */
REAL STUB, SCALE, EPS1, EPS2; // EPS1: optimize, EPS2: error
...
ITER: /* .. and optimize it until either obviously bad or small error figure happens */
			...
			if (r > EPS2*q) { optimize(EPS1*r/NUMSITES); goto ITER; }
			if ( q - r < STUB) { /* moved behind previous line!!!! */
				if (k >= NUMSITES-3) {
					do { optimize(EPS1*r/NUMSITES); q = length(); r = error(); 
						/* printf("%20.20g %20.20g\n", q, r); */
					} while (r > q*EPS1);
